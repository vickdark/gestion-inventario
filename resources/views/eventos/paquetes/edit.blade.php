...
<div class="container-fluid py-4">
    <div class="row mb-4">
        <div class="col-12 d-flex align-items-center justify-content-between">
            <div>
                ...
                <h3 class="fw-bold mb-0">Editar Paquete: {{ $paquete->nombre }}</h3>
            </div>
    ...
    <form action="{{ route('eventos.paquetes.update', $paquete->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="row g-4">
            <!-- Información Básica -->
            <div class="col-lg-5">
                <div class="card border-0 shadow-sm rounded-4 p-4">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="image-upload-wrapper bg-light rounded-4 d-flex align-items-center justify-content-center position-relative overflow-hidden border-dashed" style="height: 200px; border: 2px dashed #dee2e6;">
                            ...
                            <div class="text-center p-3 {{ $imageSrc ? 'd-none' : '' }}" id="preview-placeholder">
                                ...
                                <p class="small text-muted mb-0">Cambiar imagen</p>
                            </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control rounded-3 @error('nombre') is-invalid @enderror" id="nombre" name="nombre" value="{{ old('nombre', $paquete->nombre) }}" required>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" step="0.01" class="form-control border-start-0 rounded-end-3 @error('precio_dia') is-invalid @enderror" id="precio_dia" name="precio_dia" value="{{ old('precio_dia', $paquete->precio_dia) }}" required>
                        </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            <span class="input-group-text bg-light border-end-0 rounded-start-3">$</span>
                            <input type="number" step="0.01" class="form-control border-start-0 rounded-end-3 @error('precio_hora') is-invalid @enderror" id="precio_hora" name="precio_hora" value="{{ old('precio_hora', $paquete->precio_hora) }}" required>
                        </div>
                    ...
                    <div class="mb-0">
                        ...
                        <textarea class="form-control rounded-3" id="descripcion" name="descripcion" rows="4">{{ old('descripcion', $paquete->descripcion) }}</textarea>
                    </div>
                </div>
            </div>
                ...
                <div class="card border-0 shadow-sm rounded-4 p-4">
                    ...
                    <div class="table-responsive mb-4" style="max-height: 400px;">
                        <table class="table table-hover align-middle" id="equipos-table">
                            ...
                            <tbody>
                                ...
                                @php
                                    $pivot = $paquete->equipos->where('id', $equipo->id)->first();
                                    $isSelected = !is_null($pivot);
                                    $qty = $isSelected ? $pivot->pivot->cantidad : 1;
                                @endphp
                                <tr class="equipo-row" data-id="{{ $equipo->id }}" data-nombre="{{ strtolower($equipo->nombre) }}" data-precio-dia="{{ $equipo->precio_dia }}" data-precio-hora="{{ $equipo->precio_hora }}">
                                    <td>
                                        <div class="form-check">
                                            <input class="form-check-input equipo-checkbox" type="checkbox" name="equipos_selected[]" value="{{ $equipo->id }}" id="chk_{{ $equipo->id }}" {{ $isSelected ? 'checked' : '' }}>
                                        </div>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="input-group input-group-sm">
                                            <button class="btn btn-outline-secondary btn-qty" type="button" data-action="minus" {{ !$isSelected ? 'disabled' : '' }}>-</button>
                                            <input type="number" name="equipos[{{ $loop->index }}][cantidad]" class="form-control text-center input-qty" value="{{ $qty }}" min="1" {{ !$isSelected ? 'disabled' : '' }}>
                                            <input type="hidden" name="equipos[{{ $loop->index }}][id]" value="{{ $equipo->id }}" {{ !$isSelected ? 'disabled' : '' }} class="input-id">
                                            <button class="btn btn-outline-secondary btn-qty" type="button" data-action="plus" {{ !$isSelected ? 'disabled' : '' }}>+</button>
                                        </div>
                                    </td>
                    ...
                    <div class="d-grid">
                        <button type="submit" class="btn btn-primary btn-lg rounded-pill fw-bold shadow-sm">
                            <i class="fa-solid fa-floppy-disk me-2"></i> Actualizar Paquete
                        </button>
                    </div>
                </div>
            </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        checkboxes.forEach(chk => {
            chk.addEventListener('change', function() {
                ...
                const inputs = row.querySelectorAll('.input-qty, .input-id, .btn-qty');
