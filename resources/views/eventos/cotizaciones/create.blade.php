...
<div class="container-fluid py-4">
    <form action="{{ route('eventos.cotizaciones.store') }}" method="POST" id="quote-form">
        @csrf
        <div class="row">
            ...
            <div class="col-lg-8">
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <select name="cliente_id" id="cliente_id" class="form-select rounded-3 @error('cliente_id') is-invalid @enderror" required>
                                    ...
                                    @foreach($clientes as $cliente)
                                        <option value="{{ $cliente->id }}" {{ (old('cliente_id') == $cliente->id || ($cliente_seleccionado && $cliente_seleccionado->id == $cliente->id)) ? 'selected' : '' }}>
                            ...
                            <div class="col-md-3">
                                ...
                                <select name="tipo_alquiler" id="tipo_alquiler" class="form-select rounded-3" required>
                                    <option value="dia" {{ old('tipo_alquiler') == 'dia' ? 'selected' : '' }}>Por Día</option>
                                    <option value="hora" {{ old('tipo_alquiler') == 'hora' ? 'selected' : '' }}>Por Hora</option>
                                </select>
                            </div>
                            <div class="col-md-3">
                                <label class="form-label text-muted small fw-bold">Fecha Emisión</label>
                                <input type="date" name="fecha_emision" class="form-control rounded-3" value="{{ old('fecha_emision', date('Y-m-d')) }}" required>
                            </div>
                            <div class="col-md-3">
                                <label class="form-label text-muted small fw-bold">Fecha Vencimiento</label>
                                <input type="date" name="fecha_vencimiento" class="form-control rounded-3" value="{{ old('fecha_vencimiento', date('Y-m-d', strtotime('+15 days'))) }}" required>
                            </div>
                            <div class="col-md-4">
                                <label class="form-label text-muted small fw-bold">Inicio del Evento</label>
                                <input type="datetime-local" name="fecha_evento_inicio" class="form-control rounded-3" value="{{ old('fecha_evento_inicio') }}">
                            </div>
                            <div class="col-md-4">
                                <label class="form-label text-muted small fw-bold">Fin del Evento</label>
                                <input type="datetime-local" name="fecha_evento_fin" class="form-control rounded-3" value="{{ old('fecha_evento_fin') }}">
                            </div>
                        </div>
                ...
                <div class="card border-0 shadow-sm rounded-4">
                    <div class="card-body">
                        ...
                        <textarea name="notes" class="form-control rounded-3" rows="3" placeholder="Términos de pago, condiciones especiales..."></textarea>
                    </div>
                </div>
            </div>
                ...
                <div class="card border-0 shadow-sm rounded-4 sticky-top" style="top: 2rem;">
                    <div class="card-body p-4">
                        ...
                        <div class="d-flex justify-content-between mb-4">
                            ...
                            <span class="h5 fw-bold mb-0 text-primary" id="resumen-total">$0</span>
                        </div>

                        <button type="submit" class="btn btn-primary w-100 rounded-3 py-3 fw-bold shadow-sm">
                            <i class="fa-solid fa-save me-2"></i> Guardar Cotización
                        </button>
                        <a href="{{ route('eventos.cotizaciones.index') }}" class="btn btn-light w-100 rounded-3 py-3 fw-bold mt-2">
                            Cancelar
                        </a>
                    </div>
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Combinar equipos y paquetes para la búsqueda
        const allItems = [
